...
func TestReadConfigFile_MergedSources_Success(t *testing.T) {
	...
	err = ReadConfigurationFile(context.Background(), filepath.Join(tempTestdataPath, "testconfig.yaml"), false, appConfiguration)
	if err != nil {
		fmt.Println(err)
	}
...
func TestReadConfigFileCommaDelimitedRepositoryLists(t *testing.T) {
	...
	err := ReadConfigurationFile(context.Background(), "testdata/minimal_config.yaml", false, appConfiguration)
	if err != nil {
...
func TestDefaultConfigLoader_LoadConfiguration_Success(t *testing.T) {
	...
	cliConfig := entities.NewCLIConfigBuilder().
		WithConfigFilePath("testdata/minimal_config.yaml").
		WithConfigFileOnly(true).
		Build()
