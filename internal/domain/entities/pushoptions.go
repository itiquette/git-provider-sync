...
func (b PushOptionsBuilder) Build() (PushOptions, error) {
	if b.options.targetURL == "" {
		return PushOptions{}, domain.ErrTargetURLRequired
	}

	if len(b.options.refSpecs) == 0 {
		return PushOptions{}, domain.ErrRefspecRequired
	}

	if b.options.remoteName == "" {
		return PushOptions{}, domain.ErrRemoteNameRequired
	}
...
func sanitizeURLForLogging(url string) string {
	...
	// Reconstruct URL without auth: protocol:// + remainder after @
