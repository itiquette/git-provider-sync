...
func PrintConfiguration(appCfg dto.AppConfiguration, writer io.Writer) {
	if writer == nil {
...
func printEnvironment(name string, env dto.Environment, writer io.Writer, level int) {
...
func printSyncConfig(name string, syncCfg dto.SyncConfig, writer io.Writer, level int) { //nolint:cyclop // Configuration printing logic
	indent := strings.Repeat(" ", level*indentSize)
...
func printAuthConfig(authCfg dto.AuthConfig, writer io.Writer, level int) { //nolint:cyclop // Auth configuration printing logic
	indent := strings.Repeat(" ", level*indentSize)
	_, _ = fmt.Fprintf(writer, "\n%sAuthentication:\n", indent)
...
func printMirrorConfig(name string, mirrorCfg dto.MirrorConfig, writer io.Writer, level int) {
	indent := strings.Repeat(" ", level*indentSize)
	_, _ = fmt.Fprintf(writer, "\n%sMirror: %s\n", indent, name)
...
func printMirrorSettings(settings dto.MirrorSettings, writer io.Writer, level int) {
	indent := strings.Repeat(" ", level*indentSize)
	_, _ = fmt.Fprintf(writer, "\n%sSettings:\n", indent)
...
func printRepositoriesOption(opt dto.RepositoriesOption, writer io.Writer, level int) {
...
func isEmptyAuthConfig(authCfg dto.AuthConfig) bool {
...
func isEmptyRepositoriesOption(opt dto.RepositoriesOption) bool {
	return len(opt.Include) == 0 && len(opt.Exclude) == 0
}

func isEmptyMirrorSettings(settings dto.MirrorSettings) bool {
